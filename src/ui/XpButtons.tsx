...
export default function XpButton({ text, amount, logo, isHighlighted = false }: XpButtonProps) {
  ...
  const fillPercentage = useMemo(() => {
    if (text === 'XP') return 25;
    if (text === 'PoW') return 65;
    return 50;
  }, [text]);
  ...
  const logoContainerClasses = useMemo(() =>
    `absolute w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-xl backdrop-blur-sm border-2 transition-all duration-200 bg-gradient-to-br ${
      isHighlighted
  ...
  const logoClasses = useMemo(() =>
    `relative z-10 object-contain p-1.5 sm:p-2 transition-all duration-200 ${
      isHighlighted
  ...
  const textOverlayClasses = useMemo(() =>
    `absolute inset-0 flex items-center justify-center z-30 pointer-events-none transition-all duration-200 tracking-widest text-[10px] sm:text-xs font-semibold ${
      isHighlighted
  ...
  const amountTextClasses = useMemo(() =>
    `relative z-20 font-bold transition-all duration-200 text-xs sm:text-sm md:text-base ${
      isHighlighted
  ...
  return (
    <motion.div 
      className="relative group flex gap-2 sm:gap-3 md:gap-5 items-center w-48 sm:w-56 md:w-72 h-12 sm:h-14 md:h-16"
    ...
    >
      ...
      {/* Logo Container */}
      <div className="relative z-20 flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12">
        {/* Background */}
        <div className={logoContainerClasses}>
          {/* Top highlight */}
          <div className="absolute top-1 left-1 right-1 h-3 sm:h-4 rounded-lg bg-gradient-to-b from-white/20 via-white/10 to-transparent opacity-60" />
        </div>
      ...
      {/* Amount Container */}
      <div className="relative ml-2 sm:ml-3 md:ml-5 flex justify-center items-center h-4/5 rounded-xl overflow-hidden w-32 sm:w-40 md:w-56">
        <div className={amountContainerClasses}>
          ...
          {/* Inner highlight */}
          <div className="absolute top-1 left-2 right-2 h-2 sm:h-3 rounded-lg bg-gradient-to-b from-white/10 to-transparent opacity-40 z-10" />
        </div>
